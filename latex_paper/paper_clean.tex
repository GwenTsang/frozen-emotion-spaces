...
We evaluate decoders on the \emph{same} frozen $z$ and the \emph{same} outer/inner split lock as \S\S\ref{sec:layerwise}--\ref{sec:conditional}, differing only in which geometric parameters are fitted: prior-only (D0), nearest centroid (D1), centroid-offset diagram (D1o: D1 centroids frozen, free per-class offsets $\omega_k$, shared scale $\gamma$), ordinary Voronoi (D2, single site/class, tied $\omega$), power diagram (D3, single site/class, free $\omega_{k}$---equivalent to unconstrained logistic regression via \eqref{eq:pow}), and multi-prototype (D4, $m=2$--$3$ sites/class). D1 and D4 convert squared-distance scores to probabilities through $\operatorname{softmax}(-\gamma d_k^2)$; D1o uses $\operatorname{softmax}_k(-\gamma\|z-c_k\|^2+\omega_k)$, keeping the sites $c_k$ fixed at the D1 centroids and learning only the free offsets $\omega_k$.  In all three, $\gamma$ leaves hard nearest-site assignments unchanged but calibrates the held-out log loss. Table~\ref{tab:ladder} retains the historical D1/D4 values obtained with a fixed $\gamma$. Table~\ref{tab:ladder-diag} gives matched diagnostic reruns, in which $\gamma$, $m\in\{2,3\}$ for D4/D4c/D4d, and $C$ are selected on inner folds. D1$\to$D1o quantifies \emph{offset need} at fixed prototype geometry. Two diagnostic variants extend D4 (\S\ref{sec:ladder}): centroid-anchored $k$-means (D4c; one site per class frozen at the D1 centroid) and a discriminative multi-prototype fit (D4d; the $k$-means sites optimized by L-BFGS on the multiclass loss with log-sum-exp site aggregation, so $m{=}1$ recovers D3 exactly). All are CPU-only $L$-BFGS / $k$-means fits.\footnote{A shared-Mahalanobis power diagram with metric $M$, free sites $p_k$, and free offsets $\omega_k$ assigns class scores $-(z-p_k)^\top\!M(z-p_k)+\omega_k$. Because the quadratic term $z^\top\!Mz$ is common to all $k$, the decision rule reduces to $\arg\max_k\,(2p_k^\top\!Mz - p_k^\top\!Mp_k + \omega_k)$, which is affine in $z$---algebraically equivalent to D3. Introducing $M$ therefore changes the parametrisation, not the class of realisable decision boundaries.}
...
\textbf{Estimator diagnostics.} We ran targeted variants under the same nested protocol, with the distance scale $\gamma$ selected on inner folds for all distance-based decoders (Table~\ref{tab:ladder-diag}). The centroid-offset decoder D1o improves on D1 by $0.019$~bits on $A$ (95\% CI $[-0.026, -0.013]$) and $0.023$~bits on $H$ (95\% CI $[-0.029, -0.017]$), confirming that offsets carry marginal information when sites are held fixed. Under this matched calibration, D2/D3 levels are unchanged within 0.02~bits of Table~\ref{tab:ladder}, and unsupervised D4 remains significantly worse than D3 in both spaces ($+0.176$ [$0.152, 0.201$] on $A$; $+0.595$ [$0.556, 0.631$] on $H$); it is additionally worse than D1 in the hidden-state space ($2.793$ vs.\ $2.591$~bits, paired group-bootstrap 95\% CI for $\Delta$~$[0.173, 0.228]$). First, \emph{centroid anchoring} (D4c: one site per class frozen at the D1 centroid, $k$-means refining only the remaining $m{-}1$ sites) does not recover the loss of the discriminative D3 fit; second, \emph{discriminative refinement} (D4d: same $k$-means initialisation refined on the classification loss with log-sum-exp aggregation) matches D3 on $H$ ($2.198$ vs.\ $2.197$~bits) and significantly improves on it in the appraisal space ($2.436$ vs.\ $2.478$~bits, 95\% CI for $\Delta$~$[-0.054, -0.029]$), though whether this reflects genuine multi-cell structure or residual capacity effects requires confirmatory designs in future work.
...
\begin{table}[t]
\centering\footnotesize
\caption{Decoder diagnostics on crowd-enVENT under the same nested protocol as Table~\ref{tab:ladder} (5 outer / 3 inner folds; $\gamma$, $m\in\{2,3\}$, and $C$ selected on inner folds; $\pm$ = fold SD). D1o freezes sites at D1 centroids and fits free offsets $\omega_k$. D4c freezes one site per class at the class centroid; D4d refines the same $k$-means initialization on the classification loss. Unlike Table~\ref{tab:ladder}, $\gamma$ is inner-tuned for all distance-based decoders here. It calibrates log loss without changing nearest-site hard assignments, so absolute levels are not compared across tables; D2/D3 agree within $0.02$~bits. Paired group-bootstrap intervals for the key within-table contrasts are reported in the text.}
...
Centroid-offset (D1o) & $2.698\pm0.055$ & $2.569\pm0.063$ \\
