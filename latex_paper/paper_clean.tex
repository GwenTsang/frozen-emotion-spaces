...
\title{Emotion Categories in Appraisal and Transformer Spaces\\[6pt]
{\large Leakage-Controlled Frozen Probing and Geometric Decoder Tests}}
\author{}
\date{}
...
A linear probe is algebraically equivalent to a power diagram---a convex tessellation of the representation space. This geometric rewriting invites comparison with conceptual-space theories that identify natural concepts with convex regions of a similarity space \citep{gardenfors2000conceptual,douven-gardenfors-2020}. We stress from the outset that decoder geometry is not psychological geometry: the equivalence is a mathematical property of the classifier, not evidence for psychological convexity, prototypes, or a metric. What it does provide is a concrete, testable bridge: if further empirical constraints (similarity judgments, typicality, learnability) were added, the decoder's geometric parameters could be compared with those predicted by conceptual-space accounts. This paper establishes the decoder-side facts and takes a first empirical step toward the learnability bridge via Contrast and Representation scores \citep{douven-2023-naturalness}; the full psychological validation remains future work.

Four questions structure the investigation:
\begin{itemize}
...
\item[\emph{Q4.}] \emph{Design optimality:} do Contrast and Representation scores of Voronoi partitions predict held-out learnability?
\end{itemize}
...
The reported group-bootstrap intervals have nominal 95\% coverage for their individual paired contrasts; no simultaneous, across-table multiplicity adjustment is claimed. They are therefore per-contrast uncertainty statements, not a global pass/fail criterion. The explicitly exploratory Contrast/Representation analysis is interpreted separately (\S\ref{sec:contrast}).
...
\end{equation}
Thus $\omega_k$ acts as a site-specific additive advantage: increasing $\omega_k$ lowers the score of site $k$ uniformly over the space and can expand its cell. Ordinary Voronoi diagrams are the special case in which all $\omega_k$ are equal (in particular, all zero); only weight differences matter, since adding a common constant to all $\omega_k$ leaves the partition unchanged. The decoder ladder of \S\ref{sec:ladder} tests precisely this passage from tied to free $\omega_k$.\footnote{When $\omega_k\geq0$, one may write $\omega_k=r_k^2$ and interpret the construction through the classical power of a point with respect to spheres of radius $r_k$, hence the name \emph{power diagram}. We do not use this representation because it is non-canonical for our purposes: the gauge freedom $(\omega_1,\dots,\omega_K)\sim(\omega_1+c,\dots,\omega_K+c)$ maps one and the same tessellation to infinitely many radius collections---choosing $c$ large enough to make all weights nonnegative always yields \emph{some} such radii---while the objects of interest here are the class regions themselves.}
...
\begin{tikzpicture}[scale=1.05,
  ...
  \draw[<->,gray] (1.263,1.55) -- (0.946,1.605)
...
\end{equation}
Equation~\eqref{eq:voronoi} is exactly the power-diagram assignment rule of \eqref{eq:powcell} with sites and weights given by \eqref{eq:pow}: every multiclass linear probe induces a power diagram in the preprocessed coordinate system, and conversely. Each class region is a (possibly empty) convex polyhedron; boundaries are hyperplanes. The equivalence is algebraic, not evidence that categories are psychologically convex or prototype-based \citep{gardenfors2000conceptual}. However, it places the decoder ladder in correspondence with the design-criteria framework of \citet{douven-gardenfors-2020}, who argue that natural concepts are cells of optimally partitioned similarity spaces satisfying parsimony, informativeness, and learnability; independent computational evidence shows that convex (Voronoi) partitions are learned significantly faster than merely connected ones \citep{douven-verheyen-2026,douven-2023-naturalness}.
...
We evaluate decoders on the \emph{same} frozen $z$ and the \emph{same} outer/inner split lock as \S\S\ref{sec:layerwise}--\ref{sec:conditional}, differing only in free parameters, ordered by expressive power: prior-only (D0), nearest centroid (D1), ordinary Voronoi (D2, single site/class, tied $\omega$), power diagram (D3, single site/class, free $\omega_{k}$---equivalent to unconstrained logistic regression via \eqref{eq:pow}), and multi-prototype (D4, $m=2$--$3$ sites/class). D1 and D4 convert squared-distance scores to probabilities through $\operatorname{softmax}(-\gamma d_k^2)$: $\gamma$ leaves hard nearest-site assignments unchanged but calibrates the held-out log loss. Table~\ref{tab:ladder} retains the historical D1/D4 values obtained with a fixed $\gamma$. Table~\ref{tab:ladder-diag} gives matched diagnostic reruns, in which $\gamma$, $m\in\{2,3\}$ for D4/D4c/D4d, and $C$ are selected on inner folds. D2$\to$D3 quantifies \emph{offset need}. Two diagnostic variants extend D4 (\S\ref{sec:ladder}): centroid-anchored $k$-means (D4c; one site per class frozen at the D1 centroid) and a discriminative multi-prototype fit (D4d; the $k$-means sites optimized by L-BFGS on the multiclass loss with log-sum-exp site aggregation, so $m{=}1$ recovers D3 exactly). All are CPU-only $L$-BFGS / $k$-means fits.\footnote{A shared-Mahalanobis power diagram with metric $M$, free sites $p_k$, and free offsets $\omega_k$ assigns class scores $-(z-p_k)^\top\!M(z-p_k)+\omega_k$. Because the quadratic term $z^\top\!Mz$ is common to all $k$, the decision rule reduces to $\arg\max_k\,(2p_k^\top\!Mz - p_k^\top\!Mp_k + \omega_k)$, which is affine in $z$---algebraically equivalent to D3. Introducing $M$ therefore changes the parametrisation, not the class of realisable decision boundaries.}

\begin{table}[t]
\centering\footnotesize
\caption{Decoder ladder on crowd-enVENT (5 outer / 3 inner group-aware folds; $T_{\mathrm{train}}$ fitted train-only). Log loss is held-out cross-entropy; code length $\approx N\!\times\!L$. $\Delta$ vs.\ prior is negative when saving bits. Incremental $2\!\to\!3$ = offset need (Voronoi $\to$ power). $N=6600$, $K=13$. Shared-Mahalanobis variants are algebraically equivalent to D3 in decision boundaries (see footnote in text) and omitted.}
\label{tab:ladder}
\resizebox{\textwidth}{!}{%
\begin{tabular}{lccccc}
\toprule
Decoder & $L$ (nats) & $L$ (bits) & Total (kbits) & $\Delta$ vs prior (bits) & Incl.\ $2\!\to\!3$ (bits) \\
\midrule
\multicolumn{6}{c}{\textit{Appraisal $A$ 21D (standardized)}} \\
Prior-only (D0) & $2.544\pm0.002$ & $3.671\pm0.003$ & 24.23 & 0.000 & -- \\
Nearest-centroid (D1) & $2.032\pm0.006$ & $2.931\pm0.009$ & 19.34 & --0.740 & -- \\
Learned Voronoi (D2) & $1.719\pm0.017$ & $2.480\pm0.025$ & 16.38 & --1.190 & \multirow{2}{*}{0.004} \\
Power diagram (D3) & $1.716\pm0.017$ & $2.476\pm0.024$ & 16.35 & --1.195 & \\
Multi-prototype (D4, $m=2$--$3$) & $2.061\pm0.008$ & $2.973\pm0.011$ & 19.62 & --0.698 & -- \\
\midrule
\multicolumn{6}{c}{\textit{Frozen $H$ RoBERTa L12, 768D $\to$ PCA 64 (train-only)}} \\
Prior-only (D0) & $2.544\pm0.002$ & $3.671\pm0.003$ & 24.23 & 0.000 & -- \\
Nearest-centroid (D1) & $2.146\pm0.006$ & $3.096\pm0.009$ & 20.43 & --0.575 & -- \\
Learned Voronoi (D2) & $1.537\pm0.027$ & $2.217\pm0.040$ & 14.65 & --1.454 & \multirow{2}{*}{0.009} \\
Power diagram (D3) & $1.530\pm0.028$ & $2.208\pm0.041$ & 14.59 & --1.463 & \\
Multi-prototype (D4) & $2.117\pm0.007$ & $3.055\pm0.010$ & 20.16 & --0.616 & -- \\
\bottomrule
\end{tabular}%
}
\end{table}
...
\textbf{Ordinary Voronoi $\approx$ power diagram.} For $A$ 21D, D2--D3 differ by $0.004$~bits/label (0.003~nats); for $H$ PCA64, by $0.009$~bits (0.006~nats), both within standard error (Table~\ref{tab:ladder}). The equal-weight (tied-$\omega$) restriction is not rejected: the additional geometric freedom afforded by power weighting---parallel translation of pairwise Voronoi boundaries through class-specific offsets, \eqref{eq:disp}---provides negligible held-out improvement. Geometrically, the boundaries of a single-prototype description barely need to be displaced from the perpendicular bisectors of the class sites.

\textbf{Historical ladder: multi-prototype $k$-means degrades relative to D3.} In the preserved historical ladder of Table~\ref{tab:ladder}, D4 ($m=2$--$3$ per class, $k$-means with 5 restarts) \emph{increases} held-out code length by 0.50~bits ($A$) and 0.85~bits ($H$) relative to D3. With that table's fixed distance-scale calibration, D4 in the appraisal space also does not improve over D1 (2.973 vs.\ 2.931~bits), a classifier with no free parameters beyond the class centroids. Because $\gamma$ calibrates log loss, this D4/D1 ordering is protocol-specific; the robust question is whether the degradation relative to D3 reflects genuine single-cell structure or a systematic misalignment between the unsupervised $k$-means site estimator---which minimizes intra-class reconstruction error and has no mechanism to adjust sites with respect to adjacent classes---and the discriminative geometry of the task.

\textbf{Estimator diagnostics.} We ran two targeted variants under the same nested protocol, with the distance scale $\gamma$ selected on inner folds for all distance-based decoders (Table~\ref{tab:ladder-diag}). Under this matched calibration, D2/D3 levels are unchanged within 0.02~bits of Table~\ref{tab:ladder}, and unsupervised D4 remains significantly worse than D3 in both spaces ($+0.176$ [$0.152, 0.201$] on $A$; $+0.595$ [$0.556, 0.631$] on $H$); it is additionally worse than D1 in the hidden-state space ($2.793$ vs.\ $2.591$~bits, paired group-bootstrap 95\% CI for $\Delta$~$[0.173, 0.228]$). First, \emph{centroid anchoring} (D4c: one site per class frozen at the D1 centroid, $k$-means refining only the remaining $m{-}1$ sites) does not recover the loss---it degrades it further ($A$: $2.740$ vs.\ $2.654$, $+0.086$ [$0.064, 0.107$]; $H$: $3.124$ vs.\ $2.793$, $+0.331$ [$0.289, 0.371$])---arguing against a simple initialization-\emph{position} explanation relative to the D1 solution. Second, \emph{discriminative refinement} (D4d: the \emph{same} $k$-means sites optimized by L-BFGS on the multiclass softmax loss, with site scores aggregated per class by log-sum-exp, so that $m{=}1$ is an exact special case containing D3) eliminates the degradation entirely: D4d matches D3 on $H$ ($2.198$ vs.\ $2.197$~bits, $\Delta = +0.001$ [$-0.016, +0.018$]) and significantly improves on D3 on $A$ ($2.436$ vs.\ $2.478$~bits, $\Delta = -0.041$ [$-0.054, -0.029$]). This pattern strongly implicates the unsupervised reconstruction objective rather than the number of prototypes per se: multi-prototype structure does not hurt once sites are fit discriminatively, and in the appraisal space it helps.

Site placement corroborates the mechanism. Relative to the D3 decision boundaries, unsupervised $k$-means sites sit inside their own class region (median signed margin $+0.12$ in standardized units, five outer folds), centroid-anchored sites deeper still ($+0.23$), while discriminatively refined sites move onto the boundaries (median $-0.10$; 63\% lie beyond their class side of at least one boundary, as do D3's own fitted sites; Figure~\ref{fig:multiprot-sites}).

\begin{table}[t]
\centering\footnotesize
\caption{Multi-prototype estimator diagnostics on crowd-enVENT under the same nested protocol as Table~\ref{tab:ladder} (5 outer / 3 inner folds; $\gamma$, $m\in\{2,3\}$, and $C$ selected on inner folds; $\pm$ = fold SD). D4c freezes one site per class at the class centroid; D4d refines the same $k$-means initialization on the classification loss. Unlike Table~\ref{tab:ladder}, $\gamma$ is inner-tuned for D1/D4/D4c here. It calibrates log loss without changing nearest-site hard assignments, so absolute D1/D4 levels are not compared across tables; D2/D3 agree within $0.02$~bits. Paired group-bootstrap intervals for the key within-table contrasts are reported in the text.}
\label{tab:ladder-diag}
\begin{tabular}{lcc}
\toprule
Decoder & $A$ 21D, $L$ (bits) & $H$ PCA64, $L$ (bits) \\
\midrule
Nearest-centroid (D1) & $2.717\pm0.051$ & $2.591\pm0.061$ \\
Power diagram (D3) & $2.478\pm0.051$ & $2.197\pm0.081$ \\
Multi-prototype $k$-means (D4) & $2.654\pm0.053$ & $2.793\pm0.078$ \\
\quad centroid-anchored (D4c) & $2.740\pm0.054$ & $3.124\pm0.144$ \\
\quad discriminative refinement (D4d) & $2.436\pm0.048$ & $2.198\pm0.078$ \\
\bottomrule
\end{tabular}
\end{table}
...
This is a pilot over two corpora and three base-scale families, not a model-zoo survey. crowd-enVENT labels are elicited by category prompts; direct lexical expressions are masked automatically and imperfectly. The 0.28--0.47~bit conditional gain is conditional on the writer being the rater for both $A$ and $Y$ and conflates text-unexpressed appraisal content with common-rater method variance. The writer-to-reader control addresses this common-rater scale-use component but not shared elicitation dependency: both writer labels and appraisals are responses to the assigned target-emotion prompt, and the inventory measures emotion-relevant evaluations. The Contrast/Representation test (\S\ref{sec:contrast}) is exploratory: it lacks locked new-seed analysis and coefficient uncertainty. Interval estimates throughout are per-contrast 95\% bootstrap intervals rather than multiplicity-adjusted simultaneous coverage; a preregistered multiplicity policy is required for a confirmatory design. No result establishes psychological reality, a metric or convexity claim, causal use by the Transformer, or natural-kind status. The strongest direct statement is that a category label is more or less linearly accessible in a named frozen representation under the locked split, masking, pooling, capacity, and selection protocol. Exploratory cross-corpus VAD comparisons are indirect (different label sets).

\section{Perspectives}

We consolidate future work into four falsifiable directions.

\textbf{P1.\ Confirmatory design-optimality test.} The exploratory Contrast/Representation results (\S\ref{sec:contrast}) should be replicated under a confirmatory design that freezes complete-group budgets and learner parameters before seeing results; retains repetitions rather than averaging them; estimates coefficient uncertainty with constellation-level clustering or a multilevel model; reports the Contrast--Representation collinearity; and compares observed centroids with support-preserving label permutations and centroids of counterfactual cells under a preregistered fidelity criterion. Correlating Contrast and Representation with held-out code length from the decoder ladder (Table~\ref{tab:ladder}) across folds would directly test whether design-optimality criteria predict probe compression. Independent similarity/typicality data \citep{nosofsky1986attention,shepard1987toward} are required to move from decoder geometry to a psychological conceptual space.

\textbf{P2.\ Disentangling broad affect from appraisal.} A within-corpus test with items bearing both VAD and 21D appraisals would compare VAD$\to Y$, $H\to Y$, [VAD;$H$]$\to Y$, $A\to Y$, and [A;$H$]$\to Y$ to test whether the appraisal residual is reducible to VAD.

\textbf{P3.\ Removing the common-rater confound.} An adequately powered $A_{\mathrm{reader}}\!\to\!Y_{\mathrm{reader}}$ design with disjoint, larger reader panels for coordinates versus labels would isolate appraisal content from scale-use consistency.

\textbf{P4.\ Preregistered replication.} The locked split, masking, pooling, capacity, and selection protocol supports a preregistered replication on held-out writers/conversations and additional base-scale families to test layerwise and conditional claims out of sample.

\section{Conclusion}

Under strict leakage control, emotion categories are linearly accessible mid-to-late in frozen Transformer representations; writer appraisals add 0.28--0.47~bits of conditional information beyond frozen text (the residual observed for 21D appraisals on crowd-enVENT is substantially larger than the $\sim$0.06-bit VAD residual on EmoTwiCS, though whether this difference reflects representational richness or corpus/task differences requires a within-corpus comparison); that gain is largely low-rank---the top three appraisal components recover 67--77\% of it---yet carries a statistically significant fine-grained residual through ten components; a single-prototype power-diagram decoder is the shortest unsupervised description, and targeted diagnostics show that the multi-prototype degradation under $k$-means is an estimator failure---eliminated by a discriminative multi-prototype fit, which significantly improves on D3 in appraisal space---rather than evidence for single-cell geometry; and an exploratory counterfactual-partition test shows that greater Contrast and smaller Representation distance predict better held-out learnability in both appraisal and frozen-text spaces, though observed labels occupy an extreme design trade-off rather than jointly dominating random constellations. These are protocol-relative, decoder-relative facts that motivate, but do not themselves establish, a psychological conceptual-space account.

The exploratory Q4 associations are modest ($R^2=0.04$--$0.30$) but sign-consistent; Contrast and Representation remain collinear and are not identified as separable causal effects.

\paragraph{Code and data availability.} Source code for the public clean-room reconstruction, locked splits, tests, protocols, and content-addressed replication artifacts is available at \url{https://github.com/GwenTsang/frozen-emotion-spaces}. The repository does not redistribute corpus text; it documents how to obtain the original corpora under their respective terms, fetch locked numerical artifacts, and rerun each authenticated analysis. Historical values retained for comparison are explicitly distinguished from new replications.
